...
function getRouteFromHash(): AppRoute {
  switch (window.location.hash) {
    case '#onboarding_01': return { page: 'onboarding_01' }
    case '#onboarding_02': return { page: 'onboarding_02' }
    case '#onboarding_03': return { page: 'onboarding_03' }
    case '#onboarding_04': return { page: 'onboarding_04' }
...
function getHashForRoute(page: CurrentPage): string {
  switch (page) {
    case 'onboarding_01': return '#onboarding_01'
    case 'onboarding_02': return '#onboarding_02'
    case 'onboarding_03': return '#onboarding_03'
    case 'onboarding_04': return '#onboarding_04'
...
export function App() {
  ...
  const [workspace, setWorkspase] = useState(null);
  ...
  const hadleNavigate = (page: CurrentPage): void => {
  ...
  const handleSkip = (): void => { hadleNavigate('login'); }
  ...
  return (
    <main className='app_shell' aria-label='fitnote workspace'>
      {currentPage === 'splash' ? (
        <SplashPage onNext={() => hadleNavigate('onboarding_01')} />) : null}
      {currentPage === 'onboarding_01' ? (
        <Onboarding01Page onNext={() => hadleNavigate('onboarding_02')} onSkip={handleSkip} />) : null}
      {currentPage === 'onboarding_02' ? (
        <Onboarding02Page onNext={() => hadleNavigate('onboarding_03')} onSkip={handleSkip} />) : null}
      {currentPage === 'onboarding_03' ? (
        <Onboarding03Page onNext={() => hadleNavigate('onboarding_04')} onSkip={handleSkip} />) : null}
      {currentPage === 'onboarding_04' ? (
        <Onboarding04Page onNext={() => hadleNavigate('login')} />) : null}
      {currentPage === 'login' ? <LoginPage onJoin={() => hadleNavigate('signup')} onAuthenticated={handleAuthenticated} /> : null}
    </main>
  )
}
